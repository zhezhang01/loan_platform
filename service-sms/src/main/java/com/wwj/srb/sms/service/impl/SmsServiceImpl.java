...
public class SmsServiceImpl implements SmsService {
    ...
    public void send(String mobile, String templateCode, Map<String, Object> param) {
        ...
        // Request parameters for creating a remote connection
        ...
        try {
            // Use the client object to carry the request parameters to initiate a remote call to the Alibaba cloud server
            ...
            // SMS frequently sent. Error handling is carried out. By default, only one SMS can be sent within one minute
            Assert.notEquals("isv.BUSINESS_LIMIT_CONTROL", code, ResponseEnum.ALIYUN_SMS_LIMIT_CONTROL_ERROR);
            // SMS sending failed, error handling
            Assert.equals("OK", code, ResponseEnum.ALIYUN_SMS_ERROR);
        } catch (ClientException e) {
            log.error("Alicloud SMS service call failed:" + e.getErrCode() + "," + e.getErrMsg());
